...
RLUTIL_INLINE void setString(const RLUTIL_STRING_T & str_) {
	...
	wchar_t* wStr = (wchar_t*)malloc(sizeof(wchar_t) * str_.length());
	size_t nConverted = 0;
	mbstowcs_s(&nConverted, wStr, str_.length() + 1, const_cast<const char*>(str), str_.length());
...
RLUTIL_INLINE void setString(RLUTIL_STRING_T str) {
	...
	WriteConsoleOutputCharacter(hConsoleOutput, wStr, len, csbi.dwCursorPosition, &numberOfCharsWritten);
#else // _WIN32 || USE_ANSI
	RLUTIL_PRINT(str);
	#ifdef __cplusplus
